    ...
    <div class="container-fluid">
        ...
        <form enctype="multipart/form-data" action="{{ url('update-title-exe') }}" id="updateTitleForm" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="name" value="{{$titleData['name']}}" />
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="yor" , value="{{$titleData['yor']}}" />
            </div>
            <div class="mb-3">
                <label for="" class="form-label">Language</label>
                <input type="text" class="form-control" name="language" value="{{$titleData['language']}}" />
            </div>
            <div class="mb-3">
                <label for="" class="form-label">Lot</label>
                <input type="text" class="form-control" name="lot" value="{{$titleData['lot']}}" />
            </div>

            <div class="mb-3">
                <label for="" class="form-label">Grading</label>
                <input type="text" class="form-control" name="grading" value="{{$titleData['grading']}}" />
            </div>
            <h5>Rights we have : </h5>
            <div class="form-check form-check-inline">
                <input name="full_movie" class="form-check-input" type="checkbox" id="" value="YES" @if($titleData['full_movie']=='YES' ) checked @endif />
                <label class="form-check-label" for="">Full Movie</label>
            </div>
            <div class="form-check form-check-inline">
                <input name="scene" class="form-check-input" type="checkbox" id="" value="YES" @if($titleData['scene']=='YES' ) checked @endif />
                <label class="form-check-label" for="">Scene</label>
            </div>
            <div class="form-check form-check-inline">
                <input name="song" class="form-check-input" type="checkbox" id="" value="song" @if($titleData['song']=='YES' ) checked @endif />
            ...
            <div class="row m-0">
                ...
                <div class="form-check form-check-inline col-md-3 m-0 mb-2">
                    <input name="channels[]" class="form-check-input" type="checkbox" value="{{$channel['id']}}" @if(in_array($channel['id'],$channel_ids_with_rights)) checked @endif />
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.result == "success") {
                    $("span#success-toast-message").html(response.message);
